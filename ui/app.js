...
function settingsPage() {
  ...
  return `<section><div class="page-heading"><div><h1>设置</h1><p>纯浏览器扩展，不连接外部服务</p></div></div><div class="settings-grid">
    ...
    <div class="settings-panel"><h2>打开方式</h2><label class="setting-row"><span>点击扩展图标时</span><select name="actionLaunchMode" data-setting><option value="sidepanel" ${settings.actionLaunchMode === "sidepanel" ? "selected" : ""}>打开侧边栏</option><option value="page" ${settings.actionLaunchMode === "page" ? "selected" : ""}>打开完整页面</option></select></label></div>
  </div></section>`;
}
...
function playerMarkup() {
  ...
  const originalUrl = bilibiliVideoUrl(track);
  const originalHint = track ? `在哔哩哔哩打开原作品：${track.title}` : "";
  const cover = image(track?.cover, track?.title ?? "尚未播放", "now-cover");
  const coverMarkup = originalUrl
    ? `<a class="now-cover-link" href="${escapeHtml(originalUrl)}" target="_blank" rel="noopener noreferrer" title="${escapeHtml(originalHint)}" aria-label="${escapeHtml(originalHint)}">${cover}</a>`
    : cover;
  const titleMarkup = originalUrl
    ? `<a class="now-title now-title-link" href="${escapeHtml(originalUrl)}" target="_blank" rel="noopener noreferrer" title="${escapeHtml(originalHint)}">${escapeHtml(track.title)}</a>`
    : `<div class="now-title">${escapeHtml(track?.title ?? "选择一个作品开始播放")}</div>`;
  return `<footer class="player-bar">
    <div class="now-playing">${coverMarkup}<div class="now-copy">${titleMarkup}<div class="now-meta"><span class="muted">${escapeHtml(canResume ? `上次播放至 ${formatDuration(player.currentTime)}${resumeContext ? ` · ${resumeContext}` : ""}` : track?.creator?.name ?? "哔哩音频")}</span>${sourceLabel ? `<span class="source-badge ${player.source?.kind === "cache" ? "local" : ""}">${escapeHtml(sourceLabel)}</span>` : ""}${player.error ? `<span class="player-error">${escapeHtml(player.error)}</span>` : ""}</div></div></div>
